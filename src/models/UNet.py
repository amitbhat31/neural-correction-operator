...
class UNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.trunk[f'mid_fc'] = nn.Linear(embed_dim, mid[b][1])
        self.trunk[f'mid_groupnorm'] = nn.GroupNorm(mid[b][4], num_channels = mid[b][1])

        for b in range(1, conf.num_blocks):
            self.trunk[f'up_conv_{b}'] = nn.ConvTranspose2d(up[b][0], up[b][1], kernel_size=up[b][2], stride=up[b][3], bias=False)
            self.trunk[f'up_fc_{b}'] = nn.Linear(embed_dim, up[b][1])
            self.trunk[f'up_groupnorm_{b}'] = nn.GroupNorm(up[b][4], num_channels = up[b][1])
        
        self.trunk['up_conv_0'] = nn.ConvTranspose2d(up[0][0], up[0][1], kernel_size=up[0][2], stride=up[0][3], bias=False)
    ...
    def forward(self, x, x_hat, t):
        ...
        x0 += self.trunk['down_fc_0'](x)[..., None, None]
        x0 = self.trunk['down_groupnorm_0'](x0)
        x0 = self.act(x0)
        
        x1 = self.trunk['down_conv_0'](x0)
        x1 += self.trunk['down_fc_0'](embed)[..., None, None]
        x1 = self.trunk['down_groupnorm_0'](x1)
        x1 = self.act(x1)
        
        x2 = self.trunk['down_conv_0'](x1)
        x2 += self.trunk['down_fc_0'](embed)[..., None, None]
        x2 = self.trunk['down_groupnorm_0'](x2)
        x2 = self.act(x2)
        ...
        x_up = torch.cat([x_up, x_1], dim=1)
        x_up = self.trunk['up_conv_1'](x_mid)
        x_up += self.trunk['up_fc_1'](embed)[..., None, None]
        x_up = self.trunk['up_groupnorm_1'](x_up)
        x_up = self.act(x_up)
        
        x_up = torch.cat([x_up, x_0], dim=1)
        x_up = self.trunk['up_conv_0'](x_mid)
